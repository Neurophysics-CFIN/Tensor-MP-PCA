...
% data with noise window: window to pass over data with. Typically voxels
% -- for instance
%         window = [5 5] would process patches of data with dimensions
%         5x5x...
% varargin: is specified as name-value pairs (i.e. ...,'mask',mask,...)
%             indices: determines tensor-struture of patches. For instance for data 
%             with 5 indices with the three first being voxels, indices = 
%             {1:3 4 5} will combine the voxels into one index and retain 
%             the others so that each patch is denoised as a three-index 
%             tensor -- indices = {1:2 3 4 5} would denoise each patch as a 
%             four-index tensor and so on. It defaults to combining the 
%             voxel/window indices and sorting according to index 
%             dimensionality in ascending order, since this appears to be
%             optimal in most cases.
%             mask: if a logical mask is specified, locations were the sliding 
%             window contains no "true" voxels are skipped.
%             opt_shrink: uses optimal shrinkage if true (default is false)
%             sigma: specifies a know value for the noise sigma rather than
%             estimating it using MP-distribution
% Output:
...
% determine default index ordering (same order as input data and all of them with window indices combined in one)
...
% index addition vector (indices of voxels within sliding window reltive to corner index)
...
for i = 1:num_vox
    ...
    % simply skip to next position if this one does not correspond to correct stride
